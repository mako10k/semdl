...
- Use this decision priority when choosing a path: explicit user instruction, authoritative documents and accepted ADRs, architectural correctness, spec-derived test-first acceptance design, then local implementation convenience.
- Do not prefer the shortest local path when it bypasses unresolved spec questions, weakens test derivation from authoritative artifacts, or preserves a convenient but ungrounded implementation shape.
