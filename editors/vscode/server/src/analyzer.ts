...
function analyzeSemdlDocument(document: TextDocument, allowedTopLevelKinds: Set<string>, requireDocumentFirst: boolean): AnalysisResult {
  ...
  for (const line of buildLineInfos(document)) {
      ...
      if (stack.length === 0) {
        ...
        if (kind === 'document') {
          if (seenDocumentBlock || seenTopLevelBlock) {
            diagnostics.push(makeDiagnostic(lineRange(line), 'The `document` block is only allowed once and must appear first.', DiagnosticSeverity.Error));
            continue;
          }
    ...
    const fieldMatch = trimmed.match(fieldLinePattern);
    if (!fieldMatch || !scalarValuePattern.test(fieldMatch[3].trim())) {
      diagnostics.push(makeDiagnostic(lineRange(line), 'Expected a scalar field line in the form `name: value`.', DiagnosticSeverity.Error));
    }
  }
...
function analyzeSsqDocument(document: TextDocument): AnalysisResult {
  ...
  for (const line of buildLineInfos(document)) {
    ...
    if (seenEntries.has(key)) {
      diagnostics.push(makeDiagnostic(lineRange(line), `Duplicate query entry \`${key}\`.`, DiagnosticSeverity.Error));
    } else {
      seenEntries.set(key, line.lineNumber);
    }
    ...
    if (order < lastEntryOrder) {
      diagnostics.push(makeDiagnostic(lineRange(line), 'Query entries must appear in `select`, `where`, `similar`, `return` order.', DiagnosticSeverity.Error));
    } else {
...
function makeDiagnostic(range: Range, message: string, severity: DiagnosticSeverity): Diagnostic {
  return {
    range,
    message,
    severity,
