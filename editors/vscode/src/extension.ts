...
export async function activate(context: vscode.ExtensionContext): Promise<void> {
  ...
  output.appendLine('Current LSP slice provides richer syntax diagnostics, grammar-derived keyword completion, document-local identifier completion, grammar-derived keyword hover, and top-level document symbols.');
  ...
  const showLanguageServerStatus = vscode.commands.registerCommand('semdl.showLanguageServerStatus', async () => {
    ...
    await vscode.window.showInformationMessage('SEMDL LSP is active with richer diagnostics, keyword completion, document-local identifier completion, keyword hover, and top-level document symbols.');
  });
