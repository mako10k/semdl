...
後続の最小 apply 拡張として、`ssd normalize <input.ssd>` は standalone `.ssd` 入力に限って同一ファイルを書き換えてよい。
paired `.ssm` が存在する入力に対する bare normalize apply は、この段階では stale sidecar state を避けるため失敗とする。
