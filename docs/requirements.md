...
このとき provider は local Ollama に限定し、`--embed-provider ollama --embed-model <model>` を明示指定する。
...
初期 slice の `return: subgraph` は structural query に限定し、`select` と optional single `where` を使う通常検索でのみ受け付けることを推奨する。
`similar` と `return: subgraph` の組み合わせは、この段階では明示未実装として failure にしてよい。
...
最初の Ollama embedding slice では、`extract` は少なくとも以下を満たすこと。

- 既存 `.ssd` を入力に受けられること
- `--embed-provider ollama --embed-model <model>` を明示したときだけ embedding generation を行うこと
- `--embed-provider` と `--embed-model` は両方そろわない限り失敗すること
- `--stdout` では generated `.ssm` profile を出力できること
- `--out <file>` では出力先 `.ssd` と paired `.ssm` を生成できること
...
初期 normalize slice は `ssd normalize <input.ssd> --stdout` に限定してよく、canonical inline view を返すことを推奨する。
paired `.ssm` が存在すればそれを統合し、存在しなければ standalone `.ssd` を canonical order と formatting へ整形してよい。
